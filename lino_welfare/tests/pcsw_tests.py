...
class QuickTests(RemoteAuthTestCase):
    ...
    def test00(self):
        ...
        # ,last_name="Superuser")
        self.user_root = User(username='root', language='en', profile='900')
        self.user_root.save()
        signer1 = Person(first_name="Ernst", last_name="Keutgen")
        signer1.save()
        signer2 = Person(first_name="Joseph",
                         last_name="Ossemann")
        signer2.save()
        ...
        t = ContractType(id=1, build_method='pisa',
                         template="", name=u'Art.60\xa77')
        t.save()
...
Invalid template '' configured for ContractType 'Art.60\xa77' (expected filename ending with '.pisa.html').""")
        ...
        try:
            ...
            self.assertEqual(
                force_unicode(e), "[u'Date range overlaps with ISIP #1']")
        ...
        self.hans.full_clean()
        self.hans.save()
        r = contacts_Role(person=self.hans, company=self.job_provider, type=gf)
        r.full_clean()
        r.save()
        qs = self.jobs_contract_1.contact_person_choices_queryset(
            self.job_provider)
        ...
        i.save()
        response = self.client.get(
            '/api/notes/NoteTypes/1?fmt=json', REMOTE_USER='root')
        result = self.check_json_result(
            response, 'data title navinfo disable_delete id')
...
Belgique""")
        ...
        # Django pitfall: repr() of a model instance may return basestring
        # containing non-ascii characters.
        self.assertEqual(type(repr(a)), str)

        #
        self.assertEqual(dd.obj2str(a, True),
                         "Activity(name='Sozialhilfeempf\\xe4nger')")
        a.save()
        self.assertEqual(dd.obj2str(a, True),
                         "Activity(id=1,name='Sozialhilfeempf\\xe4nger')")
...
{ "name": "disable_editing" }"""
        ...
        response = self.client.get(
            '/api/courses/PendingCourseRequests?fmt=json', REMOTE_USER='root')
        result = self.check_json_result(
            response, 'count rows success no_data_text title param_values')
        #~ print result['rows']
        ...
        ac.full_clean()
        ac.save()
        ag = accounts.Group(name="TestGroup", chart=ac,
                            account_type=accounts.AccountTypes.expenses)
        ag.full_clean()
        ag.save()
        ...
        response = self.client.post(
            '/api/accounts/Accounts', data, REMOTE_USER='root')
        result = self.check_json_result(
            response, 'message success rows data_record')
