...
>>> theresia.profile.has_required_role(career_panel.required_roles)
False
...
So until now everything looks okay. The only problem is that the panel
actually *is* there.

>>> theresia.profile.has_required_role(career_panel.required_roles)
False

Until :blogref:`20150716`, when :meth:`write_lino_js` had run once,
then we had the requirements of our career panel modified (loosened)
